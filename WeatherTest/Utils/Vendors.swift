...
@objc public class Vendors: NSObject {
    ...
    public static func isFirstStart(version: String = "") -> Bool {
        let key: String
        if version == "" {
            key = self.BFAppHasBeenOpened
        } else {
            key = self.BFAppHasBeenOpened + "\(version)"
        }
...
public extension Vendors {
    ...
    public static func typeName(for object: Any) -> String {
        let type_ = type(of: object.self)
        return String(describing: type_)
    }
    ...
    public static func reviewURLWithId(_ appId: String) -> URL {
        return URL(string: self.reviewPathWithId(appId))!
    }
    ...
    public static func shareURLWithId(_ appId: String) -> URL {
        return URL(string: self.sharePathWithId(appId))!
    }

    public static func clearTempDirectory() {
        do {
            try self.tempContents.forEach {
    ...
    public static func deleteFileWithName(_ named: String) -> Bool {
        ...
        return self.deleteFileWithPath(path)
    }

    public var shouldDisableAnimationsViaDefaultHeuristics: Bool {
        if #available(iOS 11, *) {
